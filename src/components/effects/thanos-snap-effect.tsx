...
import { useRef, type PropsWithChildren, forwardRef, useImperativeHandle } from 'react';
import { m, useAnimate, useMotionValue, useMotionValueEvent } from 'framer-motion';
...
export const ThanosSnapEffect = forwardRef<ThanosSnapHandle, PropsWithChildren>(
  ({ children }, ref) => {
    ...
    const handleSnap = async (onComplete?: () => void) => {
      if (scope.current && scope.current.dataset.isAnimating === 'true') return;
    ...
    useImperativeHandle(ref, () => ({
      snap: (onComplete?: () => void) => {
        handleSnap(onComplete);
      }
    }));
    ...
    return (
      <div ref={scope}>
        <m.div
          ...
          className="filter-[url(#dissolve-filter)]"
        >
          {children}
        </m.div>
